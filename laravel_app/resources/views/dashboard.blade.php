    ...
    <div class="space-y-12">
        ...
        <div class="space-y-6 relative z-10">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                @foreach($predictionsByModel as $modelStat)
                    @php
                        $modelNames = [
                    ...
                    @endphp
                    <div
                        class="glass-panel p-5 rounded-2xl border {{ $color['border'] }} relative overflow-hidden group hover:shadow-lg transition-all flex flex-col h-full">
                        <div class="flex items-start justify-between mb-4">
                            <div
                                class="w-10 h-10 rounded-lg {{ $color['bg'] }} flex items-center justify-center border {{ $color['border'] }}">
                                <i data-lucide="{{ $color['icon'] }}" class="w-5 h-5 {{ $color['text'] }}"></i>
                            </div>
                            <div class="text-right">
                                <span
                                    class="text-[10px] font-black {{ $color['text'] }} uppercase tracking-widest block">{{ $modelStat->count }}x</span>
                        ...
                        <h3 class="text-sm font-black text-white mb-2">
                            {{ $modelNames[$modelStat->model_type] ?? $modelStat->model_type }}</h3>
                        ...
                        <p class="text-[10px] text-slate-500 leading-relaxed mb-4 flex-1">
                            {{ $descriptions[$modelStat->model_type] ?? 'Real-time AI analysis for road safety.' }}
                        </p>
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-white/5 mt-auto">
                            <div class="flex flex-col">
                                ...
                                <span
                                    class="text-[11px] font-black {{ $color['text'] }}">{{ round($modelStat->avg_confidence ?? 0, 1) }}%</span>
                            </div>
                            <a href="{{ Route::has($route) ? route($route) : '#' }}"
                                class="px-3 py-1.5 rounded-lg {{ $color['bg'] }} {{ $color['text'] }} text-[9px] font-black uppercase tracking-widest hover:brightness-125 transition-all flex items-center gap-1">
                ...
                @if(auth()->user()->role !== 'admin')
                    ...
                    <div
                        ...
                        <div class="flex items-start justify-between mb-4">
                            ...
                            <div class="text-right">
                                <span
                                    class="text-[10px] font-black text-orange-400 uppercase tracking-widest block">{{ number_format($carTotalExpenses) }}</span>
                    ...
                    <div
                        ...
                        <div class="flex items-start justify-between mb-4">
                            ...
                            <div class="text-right">
                                <span
                                    class="text-[10px] font-black text-teal-400 uppercase tracking-widest block">{{ $avgFleetEfficiency }}</span>
                ...
                @else
                    ...
                    <div class="glass-panel p-5 rounded-2xl border border-red-500/20 relative overflow-hidden group hover:shadow-lg transition-all flex flex-col h-full lg:col-span-2">
                        <div class="flex items-start justify-between mb-4">
                            ...
                            <div class="text-right">
                                <span class="text-[10px] font-black text-red-400 uppercase tracking-widest block">{{ $alertsCountByType->sum('count') }}x Incidents</span>
                        ...
                        <div class="flex flex-row items-center gap-8 flex-1">
                            ...
                            <div class="flex-1 space-y-3">
                                ...
                                <div class="grid grid-cols-2 gap-2">
                                    @foreach($alertsCountByType as $alertStat)
                                        <div class="flex items-center gap-2">
                                            <div class="w-1.5 h-1.5 rounded-full {{ $alertStat->type === 'Fire' ? 'bg-red-500' : ($alertStat->type === 'Accident' ? 'bg-orange-500' : 'bg-purple-500') }}"></div>
                                            <span class="text-[9px] font-bold text-slate-400 uppercase">{{ $alertStat->type }}: {{ $alertStat->count }}</span>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
        ...
        @if(auth()->user()->role === 'admin')
            ...
            <div class="mb-8 animate-fade-in-up" x-data="{ alertPage: 1, totalAlerts: {{ $latestAlerts->count() }} }">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        @php $unreadCount = \App\Models\SystemAlert::where('is_read', false)->count(); @endphp
                        @if($unreadCount > 0)
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 min-h-[160px]">
                    @forelse($latestAlerts as $index => $alert)
                        @php
                            $colors = [
                        ...
                        <a href="{{ $mapUrl }}" target="_blank" 
                           x-show="{{ $index }} >= (alertPage - 1) * 8 && {{ $index }} < alertPage * 8"
                           x-transition:enter="transition ease-out duration-300"
                           x-transition:enter-start="opacity-0 translate-x-4"
                           x-transition:enter-end="opacity-100 translate-x-0"
                           class="glass-panel p-4 rounded-2xl border {{ $c['border'] }} relative overflow-hidden group hover:shadow-lg transition-all duration-300 hover:bg-white/5">
                            <div class="flex items-start justify-between mb-2">
                                <span class="{{ $c['bg'] }} {{ $c['text'] }} px-2 py-0.5 rounded-md text-[8px] font-black uppercase tracking-widest border {{ $c['border'] }}">
                                    {{ $alert->type }}
                                </span>
                                <div class="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse mt-1"></div>
                            </div>
                            <div class="flex items-center gap-1.5 mt-2">
                                <i data-lucide="{{ $c['icon'] }}" class="w-3.5 h-3.5 {{ $c['text'] }} shrink-0"></i>
                                <div class="flex-1 min-w-0">
                                    <p class="text-[10px] font-bold text-white truncate group-hover:text-primary-400 transition-colors">{{ $alert->location_text }}</p>
                                    @if($alert->type === 'Fake Plate' && isset($alert->details['plate_text']))
                                        <p class="text-[9px] font-black text-purple-400 mt-0.5 tracking-wider">
                                            <span class="bg-purple-500/10 px-1 rounded border border-purple-500/20">{{ $alert->details['plate_text'] }}</span>
                                        </p>
                                    @endif
                                </div>
                            </div>
                            <div class="flex items-center justify-between mt-1.5">
                                <p class="text-[8px] text-slate-600 font-bold uppercase tracking-wider">{{ $alert->created_at->diffForHumans() }}</p>
                ...
                <div
                    ...
                    <div>
                        ...
                        <div class="text-5xl font-black text-white tracking-tighter">
                            <span class="text-primary-400">{{ number_format($user->balance, 0) }}</span>
                        </div>
                    </div>
                    <div
                            ...
                            <tbody class="divide-y divide-white/[0.03]">
                                @forelse($transactions as $tx)
                                    <tr class="text-slate-300 hover:bg-white/[0.01] transition-colors">
                                        <td class="py-3 pl-2 text-xs font-bold">{{ $tx->station_name }}</td>
                                        <td class="py-3 text-[11px] font-mono tracking-wider text-indigo-300">
                                            {{ $tx->plate_number }}</td>
                                        <td class="py-3 text-center text-sm font-black">{{ number_format($tx->amount, 0) }}.00</td>
                                        <td class="py-3 text-right pr-2">
                                            <span
                                                class="{{ $tx->status === 'success' ? 'bg-primary-500/10 text-primary-400 border-primary-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20' }} font-black uppercase text-[8px] tracking-[0.15em] border px-2 py-0.5 rounded-md">
                                                {{ $tx->status }}
                                            </span>
                                        </td>
                                    </tr>
                    ...
                    <div class="w-full aspect-square max-w-[140px] relative">
                        ...
                        <div class="absolute inset-0 flex items-center justify-center pointer-events-none">
                            <div class="text-center">
                                <span
                                    class="text-[10px] font-black text-white block leading-none">{{ number_format($carTotalExpenses) }}</span>
        ...
        @if($recentPredictions->isNotEmpty())
            <div class="relative z-10" x-data="{ 
                ...
                selectedPred: null,
                predPage: 1,
                totalPreds: {{ $recentPredictions->count() }},
                showImage(pred) {
                ...
                <div class="glass-panel rounded-2xl border border-white/5 overflow-hidden">
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-white/[0.03]">
                                @foreach($recentPredictions as $index => $pred)
                                    @php
                                        $modelNames = [
                                            'plate' => 'License Plates',
                                            'cracks' => 'Road Cracks',
                                    ...
                                    <tr class="text-slate-300 hover:bg-white/[0.02] transition-colors" 
                                        x-show="{{ $index }} >= (predPage - 1) * 8 && {{ $index }} < predPage * 8"
                                        x-transition:enter="transition ease-out duration-300"
                                        x-transition:enter-start="opacity-0 translate-y-2"
                                        x-transition:enter-end="opacity-100 translate-y-0">
                                        <td class="px-4 py-3">
                                            @if($pred->image_path)
                                                <div @click="showImage({{ json_encode($clientData) }})"
                                                    class="w-12 h-12 rounded-lg bg-slate-800 border border-white/10 overflow-hidden group/img relative cursor-pointer">
                                                    <img src="{{ asset('storage/' . $pred->image_path) }}" alt="Prediction"
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="flex flex-col">
                                                <span class="font-semibold {{ $colors[$pred->model_type] ?? 'text-white' }}">
                                                    {{ $modelNames[$pred->model_type] ?? $pred->model_type }}
                                                </span>
                                                <span
                                                    class="text-[9px] text-slate-500 uppercase tracking-tighter">{{ ucfirst($pred->input_type) }}</span>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 text-[11px] cursor-pointer group"
                                            @click="showResults({{ json_encode($clientData) }})">
                                        ...
                                        <td class="px-4 py-3 text-center">
                                            @if($pred->confidence_score)
                                                <div class="flex flex-col items-center">
                                                    <span
                                                        class="font-bold {{ $pred->confidence_score >= 80 ? 'text-green-400' : ($pred->confidence_score >= 60 ? 'text-yellow-400' : 'text-red-400') }}">
                                                        {{ number_format($pred->confidence_score, 1) }}%
                                                    </span>
                                                </div>
                                            @else
                                        ...
                                        <td class="px-4 py-3 text-[11px] text-slate-400">
                                            {{ $pred->created_at->format('M d, H:i') }}
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (alertsCtx) {
                new Chart(alertsCtx, {
                    type: 'doughnut',
                    data: {
                        labels: {!! json_encode($alertsCountByType->pluck('type')) !!},
                        datasets: [{
                            data: {!! json_encode($alertsCountByType->pluck('count')) !!},
                            backgroundColor: [
                                '#ef4444', // Red for Fire
