    ...
    public function detect(Request $request)
    {
        ...
        if ($is_video) {
            ...
            return new StreamedResponse(function() use ($file, $context) {
                $body = $this->mlService->streamTraffic($file);
                $buffer = '';
                while (!$body->eof()) {
                    ...
                    while (($pos = strpos($buffer, "\n")) !== false) {
                        ...
                        if ($data && isset($data['event']) && $data['event'] === 'frame') {
                            if (!empty($data['detections'])) {
                                $this->processor->process('traffic', $data, $context);
                            }
                        }
                    }
        ...
        $result = $this->mlService->detectTraffic($file);
        ...
        $result = $this->processor->process('traffic', $result, array_merge($validated, [
            'input_type' => 'image',
