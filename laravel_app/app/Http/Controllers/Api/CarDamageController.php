    ...
    public function detect(Request $request)
    {
        ...
        if ($is_video) {
            ...
            return new StreamedResponse(function() use ($file, $context) {
                $body = $this->mlService->streamCarDamage($file);
                $buffer = '';
                while (!$body->eof()) {
                    ...
                    while (($pos = strpos($buffer, "\n")) !== false) {
                        ...
                        if ($data && isset($data['event']) && $data['event'] === 'frame') {
                            if (!empty($data['detections'])) {
                                $this->processor->process('car_damage', $data, $context);
                            }
                        }
                    }
        ...
        $result = $this->mlService->detectCarDamage($file);
        ...
        $result = $this->processor->process('car_damage', $result, array_merge($validated, [
            'input_type' => 'image',
