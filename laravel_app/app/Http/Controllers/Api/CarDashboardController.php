    ...
    public function detect(Request $request)
    {
        ...
        if ($is_video) {
            ...
            return new StreamedResponse(function() use ($file, $context) {
                $body = $this->mlService->streamDashboard($file);
                $buffer = '';
                while (!$body->eof()) {
                    ...
                    while (($pos = strpos($buffer, "\n")) !== false) {
                        ...
                        if ($data && isset($data['event']) && $data['event'] === 'frame') {
                            if (!empty($data['detections'])) {
                                $this->processor->process('dashboard', $data, $context);
                            }
                        }
                    }
        ...
        $result = $this->mlService->detectDashboard($file);
        ...
        $result = $this->processor->process('dashboard', $result, array_merge($validated, [
            'input_type' => 'image',
