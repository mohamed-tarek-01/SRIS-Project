    ...
    public function detect(Request $request)
    {
        ...
        if ($is_video) {
            ...
            return new StreamedResponse(function() use ($file, $context) {
                $body = $this->mlService->streamVehicles($file);
                $buffer = '';
                while (!$body->eof()) {
                    ...
                    while (($pos = strpos($buffer, "\n")) !== false) {
                        ...
                        if ($data && isset($data['event']) && $data['event'] === 'frame') {
                            if (!empty($data['detections'])) {
                                $this->processor->process('vehicles', $data, $context);
                            }
                        }
                    }
        ...
        $result = $this->mlService->detectVehicles($file);
        ...
        $result = $this->processor->process('vehicles', $result, array_merge($validated, [
            'input_type' => 'image',
