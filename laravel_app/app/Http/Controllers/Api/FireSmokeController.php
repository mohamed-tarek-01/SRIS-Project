...
class FireSmokeController extends Controller
{
    ...
    public function detect(Request $request)
    {
        ...
        if ($is_video) {
            ...
            return new StreamedResponse(function() use ($file, $context) {
                $body = $this->mlService->streamFireSmoke($file);
                $buffer = '';
                while (!$body->eof()) {
                    ...
                    while (($pos = strpos($buffer, "\n")) !== false) {
                        ...
                        if ($data && isset($data['event']) && $data['event'] === 'frame') {
                            if (!empty($data['detections'])) {
                                $this->processor->process('fire_smoke', $data, $context);
                            }
                        }
                    }
        ...
        // Image inference
        $result = $this->mlService->detectFireSmoke($file);
        ...
        $result = $this->processor->process('fire_smoke', $result, array_merge($validated, [
            'input_type' => 'image',
