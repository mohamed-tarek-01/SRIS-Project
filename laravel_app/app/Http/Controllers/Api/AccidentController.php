    ...
    public function detect(Request $request)
    {
        ...
        if ($is_video) {
            ...
            return new StreamedResponse(function() use ($file, $context) {
                $body = $this->mlService->streamAccident($file);
                $buffer = '';
                while (!$body->eof()) {
                    ...
                    while (($pos = strpos($buffer, "\n")) !== false) {
                        ...
                        if ($data && isset($data['event']) && $data['event'] === 'frame') {
                            // Check if frame has detections and log/alert
                            if (!empty($data['detections'])) {
                                $this->processor->process('accident', $data, $context);
                            }
                        }
                    }
        ...
        // Image inference
        $result = $this->mlService->detectAccident($file);
        ...
        $result = $this->processor->process('accident', $result, array_merge($validated, [
            'input_type' => 'image',
