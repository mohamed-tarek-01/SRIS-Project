...
<div class="max-w-7xl mx-auto space-y-12 pb-20" x-data="tripCalculator()">
    ...
    <?php if($vehicles->isEmpty()): ?>
        <div class="glass-panel rounded-[2.5rem] p-12 text-center space-y-6 border border-white/5">
            ...
            <a href="<?php echo e(route('user.vehicles.index')); ?>" class="inline-flex px-8 py-4 bg-white text-dark-900 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-primary-500 hover:text-white transition-all">
                Go to Garage
            </a>
        </div>
    <?php else: ?>
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1">
                <div class="glass-panel rounded-[2.5rem] border border-white/5 p-8 sticky top-32">
                    ...
                    <form action="<?php echo e(route('user.trips.store')); ?>" method="POST" class="space-y-5" @submit="onSubmit">
                        <?php echo csrf_field(); ?>
                        <div class="space-y-2">
                            ...
                            <select name="vehicle_id" x-model="vehicleId" required class="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-emerald-500/50 transition-all appearance-none">
                                <?php $__currentLoopData = $vehicles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $vehicle): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($vehicle->id); ?>" class="bg-dark-800"><?php echo e($vehicle->brand); ?> <?php echo e($vehicle->model); ?></option>
                            ...
                            <div class="flex items-center gap-6">
                                ...
                                <div>
                                    <div class="flex items-center gap-3 mb-1">
                                        <h4 class="text-xl font-black text-white uppercase"><?php echo e($trip->origin); ?> <span class="text-slate-600 px-2">→</span> <?php echo e($trip->destination); ?></h4>
                                    </div>
                                    <p class="text-xs font-bold text-slate-500 uppercase tracking-widest">
                                        <?php echo e($trip->vehicle->brand); ?> • <?php echo e(number_format($trip->distance_km, 1)); ?> KM
                                        <?php if($trip->fuel_consumed): ?>
                                        • <span class="text-emerald-400"><?php echo e(number_format($trip->fuel_consumed, 1)); ?> L</span> (<?php echo e(number_format($trip->fuel_cost, 0)); ?> EGP)
                                        <?php endif; ?>
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-6">
                                <div class="text-right">
                                    <p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                                        <?php echo e($trip->created_at->format('M d, Y')); ?>

                                    </p>
                                </div>
                                
                                <form action="<?php echo e(route('user.trips.destroy', $trip)); ?>" method="POST" onsubmit="return confirm('Remove trip?')">
...
<script>
    const vehicleData = <?php echo json_encode($vehicleData, 15, 512) ?>;
    const PETROL_PRICE = 25.0; // EGP per Litre

    function tripCalculator() {
        return {
            vehicleId: '<?php echo e($vehicles->first()->id); ?>',
            origin: '',
            destination: '',
            distance: 0,
            fuelConsumed: 0,
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/resources/views/user/trips/index.blade.php ENDPATH**/ ?>