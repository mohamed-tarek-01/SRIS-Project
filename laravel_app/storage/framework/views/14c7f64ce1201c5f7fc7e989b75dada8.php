<nav class="fixed top-4 left-1/2 -translate-x-1/2 w-[95%] max-w-7xl z-50 transition-all duration-300">
    <div class="glass-panel rounded-2xl px-4 py-3 flex items-center justify-between shadow-[0_8px_32px_rgba(45,212,191,0.05)] border border-white/5 backdrop-blur-xl">
        ...
        <!-- Logo Section -->
        <a href="<?php echo e(route('dashboard')); ?>" class="flex items-center gap-3 group relative px-2">
                ...
                <?php $__currentLoopData = $links; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $link): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <a href="<?php echo e(route($link['route'])); ?>" 
                       class="group relative flex items-center gap-1.5 px-2.5 sm:px-3 py-2 rounded-lg transition-all duration-300 text-xs font-medium
                        ...
                        <i data-lucide="<?php echo e($link['icon']); ?>" class="w-3.5 h-3.5 shrink-0 <?php echo e(request()->routeIs($link['route']) ? 'animate-pulse-glow' : ''); ?>"></i>
                        <span class="whitespace-nowrap hidden xl:block"><?php echo e($link['label']); ?></span>
                    </a>
        ...
        <div class="flex items-center gap-2">
            <?php if(auth()->guard()->check()): ?>
                <div class="hidden md:flex flex-col items-end">
                    <span class="text-[11px] font-bold text-white leading-none"><?php echo e(auth()->user()->name); ?></span>
                    <span class="text-[9px] text-primary-400 uppercase tracking-tighter"><?php echo e(auth()->user()->role); ?></span>
                </div>
                ...
                <?php if(auth()->user()->role === 'admin'): ?>
                    <a href="<?php echo e(route('dashboard')); ?>" class="relative group/bell">
                        <div class="p-2 rounded-lg bg-white/5 border border-white/10 text-slate-400 hover:text-red-400 hover:bg-red-500/10 hover:border-red-500/20 transition-all">
                            <i data-lucide="bell" class="w-4 h-4 <?php echo e($unreadAlerts > 0 ? 'animate-bounce' : ''); ?>"></i>
                        ...
                        <div class="absolute top-full right-0 mt-2 w-40 p-2.5 bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl opacity-0 translate-y-1 pointer-events-none group-hover/bell:opacity-100 group-hover/bell:translate-y-0 transition-all z-50">
                            <p class="text-[9px] font-black text-white uppercase tracking-widest"><?php echo e($unreadAlerts); ?> Active Alerts</p>
                            <p class="text-[8px] text-slate-500 font-bold mt-1 leading-tight">Click to view dashboard</p>
                        </div>
                    </a>
                <?php endif; ?>
            <?php else: ?>
                <a href="<?php echo e(route('login')); ?>" class="px-5 py-2.5 rounded-xl bg-primary-500/10 border border-primary-500/20 text-primary-400 text-sm font-semibold hover:bg-primary-500 hover:text-white transition-all duration-300">
                    Sign In
                </a>
            <?php endif; ?>
        </div>
