...
ACCIDENT_MODEL_PATH = ROOT_DIR / "models" / "accident" / "accident_model.pt"

_accident_model: YOLO | None = None


def _load_model() -> None:
    global _accident_model
    if _accident_model is None and ACCIDENT_MODEL_PATH.exists():
        _accident_model = YOLO(str(ACCIDENT_MODEL_PATH))


def run_inference(image_bytes: bytes, filename: str | None = None) -> Dict[str, Any]:
    """
    Run accident detection using the trained YOLOv12 model.
    """
    _load_model()

    if _accident_model is None:
        return {
            "model": "accident_detection",
            "filename": filename,
            "detections": [],
            "message": "accident_model.pt file not found on disk.",
        }

    image = load_numpy_from_bytes(image_bytes)
    image_bgr = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)

    results = _accident_model.predict(source=image_bgr, verbose=False, imgsz=640)
...
def _process_frame_bgr(frame_bgr: np.ndarray, filename: str | None = None) -> Dict[str, Any]:
    ...
    results = _accident_model.predict(source=frame_bgr, verbose=False, imgsz=640)
    detections: List[Dict[str, Any]] = []
...
def run_video_inference_stream(video_bytes: bytes, filename: str | None = None):
    ...
    try:
        ...
        # Ultra-Real-Time Sampling (approx 8-10 FPS)
