...
DASHBOARD_MODEL_PATH = ROOT_DIR / "models" / "dashboard" / "dashboard_model.pt"

_dashboard_model: YOLO | None = None


def _load_model() -> None:
    global _dashboard_model
    if _dashboard_model is None and DASHBOARD_MODEL_PATH.exists():
        _dashboard_model = YOLO(str(DASHBOARD_MODEL_PATH))


def run_inference(image_bytes: bytes, filename: str | None = None) -> Dict[str, Any]:
    """
    Run dashboard warning lights detection using the trained YOLO model.
    """
    _load_model()

    if _dashboard_model is None:
        return {
            "model": "dashboard_detection",
            "filename": filename,
            "detections": [],
            "message": "dashboard_model.pt file not found on disk.",
        }
    ...
    results = _dashboard_model.predict(source=image_bgr, verbose=False, imgsz=960)
...
def _process_frame_bgr(frame_bgr: np.ndarray, filename: str | None = None) -> Dict[str, Any]:
    ...
    results = _dashboard_model.predict(source=frame_bgr, verbose=False, imgsz=960)
    detections: List[Dict[str, Any]] = []
