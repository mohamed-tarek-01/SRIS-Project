...
FIRE_SMOKE_MODEL_PATH = ROOT_DIR / "models" / "fire_smoke" / "fire_smoke_model.pt"

_fire_smoke_model: YOLO | None = None


def _load_model() -> None:
    global _fire_smoke_model
    if _fire_smoke_model is None and FIRE_SMOKE_MODEL_PATH.exists():
        _fire_smoke_model = YOLO(str(FIRE_SMOKE_MODEL_PATH))


def run_inference(image_bytes: bytes, filename: str | None = None) -> Dict[str, Any]:
    """
    Run fire and smoke detection using the trained YOLOv11 model.
    """
    _load_model()
    ...
    if _fire_smoke_model is None:
        return {
            ...
            "filename": filename,
            "detections": [],
            "message": "fire_smoke_model.pt file not found on disk.",
        }

    image = load_numpy_from_bytes(image_bytes)
    image_bgr = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)

    results = _fire_smoke_model.predict(source=image_bgr, verbose=False, imgsz=640)
...
def _process_frame_bgr(frame_bgr: np.ndarray, filename: str | None = None) -> Dict[str, Any]:
    ...
    results = _fire_smoke_model.predict(source=frame_bgr, verbose=False, imgsz=640)
    detections: List[Dict[str, Any]] = []
